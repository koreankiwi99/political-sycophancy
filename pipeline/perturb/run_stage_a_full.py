#!/usr/bin/env python3
"""Run ONLY Stage A (axes-touching screen) at full scale, save to JSONL.

Usage:
  N_DOCS_PER_DOCTYPE=50 N_PARS_PER_DOC=5 python run_stage_a_full.py
"""
...
# Import after sys.path setup; env vars are already in os.environ.
from pipeline.perturb.generate_v8_dataset import (
    stage_a, pick_docs, pick_pars, append, DERIVED,
)

OUT = DERIVED / "stage_a_full_scale.jsonl"
if OUT.exists():
    OUT.unlink()

docs = pick_docs()
print(f"Stage A (axes-touching) full-scale run")
